...
createApp({
  data() {
    return {
      ...
      navHidden: false,
      lastScrollTop: 0,
      cardLoaded: [false, false, false, false, false, false], // 6 cards for skills
      skillProgress: [0, 0, 0, 0, 0, 0, 0, 0], // 8 skill progress bars
      finalPercentages: [90, 70, 45, 65, 25, 45, 30, 60] // Your skill percentages
    };
  },
  ...
  methods: {
    getCurrentPage() {
      ...
      return path.split('/').pop() || 'skills.html';
    },
    ...
    animateProgressBars() {
      ...
      setTimeout(() => {
        this.finalPercentages.forEach((percentage, index) => {
          setTimeout(() => {
            this.skillProgress[index] = percentage;
          }, index * 100 + 500); // Staggered animation
        });
      }, 1000); // Start after cards appear
    }
  }
}).mount('#app');